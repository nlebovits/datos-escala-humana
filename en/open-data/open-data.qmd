...
for any purpose" [@opendefinition]. To clarify what this means, it might help to begin
with a couple of concrete examples.

Here are some open data portals from the U.S. government, the government of India, and
...
If you click on one of the links and explore it, you'll see many different datasets
covering different time periods, regions, and topics. You'll also likely notice that
these three portals are all government portals, which isn't an accident. The idea of
publicly-accessible data is old, but [the "open data"
...
does it offer? Can you find any data that might be useful for your work?
:::
...
What we've seen so far are examples of *data portals*, where are one of several kinds of
...
available data to find and acccess what you need.
...
As expected, we can see a title, a description, and some metadata (created date,
categories, a license, a maintainer and contact info). OpenDataPhilly also lists several
options for how to *access* the data, including a visualization page (the HTML link), an
API (more on this in a moment), and three *flat file* formats (CSV, shapefile, and
GeoJSON).
...
While both vector and raster data can come in several flat file formats--each of which
has its own strengths and limitations--tools like `geopandas` and `rasterio` make it
easy to open all of these formats with a consistent interface.
...
![What is an API? Source: [roboleary.net](https://roboleary.net/)](../../public/open-data/api-3284935437.png)
...
*change* depending on what's in the database. And while static flat files are good for
...
Geospatial Consortium (OGC)](https://www.ogc.org/) and are meant to facilitate
consistent access and distribution patterns for open geospatial data. Since the
standards and tooling are open source, they are the go-to for sharing geospatial data in
much of the world, especially the Global South.
...
![What is the cloud? [@cloudflare_cloud_definition]](../../public/open-data/what-is-the-cloud.webp)
...
need is essential; if we had to download the entirety of CMIP6 data just to run an
analysis for a small city in Bolivia, it would be literally impossible. Thanks to
cloud-native geospatial tooling, however, it is now easy to access gold-standard climate
data even on a normal office laptop.
...
![Global Land Cover Datasets Compared. Source: [Land Carbon Lab](https://landcarbonlab.org/insights/global-land-cover-maps-accuracy-applications/)](../../public/open-data/Screenshot%202026-04-09%20at%2014-54-08%20Global%20Land%20Cover%20Maps%E2%80%99%20Accuracy%20and%20Applications%20Explained.png)
...
trying to decide which [global land cover
map](https://landcarbonlab.org/insights/global-land-cover-maps-accuracy-applications/)
is best suited for use in your city, you should open it in software like QGIS and
actually compare it to high-resolution basemaps and your own understanding of what your
city is like. If you're using a global flood hazard model, compare it to a map of local
flood defenses, which are often hard to factor into global flooding models. If you're
comparing two datasets, try to understand *where* and *why* they differ. In short, do
not use data blindly.
...
estimates of flood risk. [VIIRS/FIRMS fire
observations](https://firms.modaps.eosdis.nasa.gov/map/) are helpful for mapping
wildfires at a large scale, but aren't always sensitive enough to pick up small roadside
wildfires when they've just started. Ideally, datasets would [include explicit
guidance](https://open.substack.com/pub/lettersfromthreadedfoundry/p/candid-core-framework?utm_campaign=post-expanded-share&utm_medium=web)
on what they should and shouldn't be used for, but in practice this is rare. The best
approach is to proceed with caution, look for prior precedent of how the dataset has
been applied, and consult subject matter experts whenever you have doubts. Good
geospatial work, in the end, depends as much on knowing the limits of your data as on
knowing how to process it.

## Why Use Open Data? A Case Study
...
flood hazards. In previous studies [@romanazzi2019], the city and the Universidad
Nacional de La Plata had estimated the population of these informal settlements based on
REBANAP data, Argentina's official census of informal settlements. Although RENABAP
provides the authoritative estimates of informal settlement populations, they are not
fine-grained (they simply estimate the number of families per settlement, with no
estimate of population distribution *within* settlements), and have another of
methdological limitations. The most recent data, for example, published in 2023, include
estimates based on surveys completed as far back as 2016. In the context of informal
...
footprints to compare to RENABAP data in La Plata](../cookbooks/la-plata.ipynb). Our
findings were stark: building footprints imply more than twice as many households as
RENABAP records, meaning the registry may underestimate the real population in these
settlements by more than half. In one extreme case, we found an informal settlement that
RENABAP estimated was inhabited by 330 families--but satellite imagery showed that it
contained more than *four thousand building footprints*.

![Example of Informal Settlement, La Plata, Argentina](https://nlebovits.github.io/ciut-riesgo/renabap_files/figure-html/fig-ejemplos-barrios-adicionales-output-3.png)

We ultimately estimated that as many as 100,000 people may be missing from population
surveys of La Plata's informal settlements, key information which has now been
incorporated into La Plata's first-ever municipal plan, and will be used to inform
future decisions such as flood risk planning.
...
In this chapter, we've covered what open data is, where and how to access it, and how to
think critically about data quality and fitness for use. In the next chapter, we'll
delve into climate risk data specifically, including key sources, and how to calculate
key indicators such as vulnerability, exposure, hazard, and risk.
