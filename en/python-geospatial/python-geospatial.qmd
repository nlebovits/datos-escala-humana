...
- Load and work with vector (points, lines, polygons) and raster (gridded) geospatial data in Python
- Reproject data between coordinate reference systems appropriate for your study area
- Perform spatial operations including clipping, aggregation, and zonal statistics
- Extract meaningful summaries from geospatial datasets to inform climate risk decisions
...
We load municipal boundaries for Buenos Aires Province from ARBA[@arba_partidos_pba]. The code below loads a local copy of the data; in [chapter two](/en/open-data/open-data.qmd), we'll show how to query these same data from ARBA's WFS service.
...
Transforming data from one coordinate system to another is frequently necessary when working with multiple data sources. When performing spatial analysis, all datasets must be in the same CRS. Both vector and raster data can be reprojected. In the code above, we reprojected the partidos data to EPSG:5348 (POSGAR 2007 / Argentina 4) using GeoPandas' `to_crs()` method. This ensures our data is in a projected coordinate system with units in meters, which is appropriate for spatial calculations in the La Plata region.
...
dsm.plot(robust=True, cmap="terrain", figsize=(12, 12))
```
...
When we pulled in the DEM data, we used the line `dsm_clipped = dsm.rio.clip(aoi.geometry)` to "clip" the raster to the boundaries of our area of interest (i.e., the boundaries of La Plata). **Spatial clipping** refers to extracting data (raster or vector) within a specific area of interest. Let's take a look at another example.
...
overview.plot(figsize=(12, 10), cmap=lc_cmap, norm=lc_norm, add_colorbar=False)
```

However, we're only interested in the data for the La Plata area, so we're going to clip the Mapbiomas raster to our area of interest. Because `rioxarray` is cloud-optimized, all this is done lazily, which means we're only actually loading in the bytes corresponding to our area of interest.
...
buildings_in_la_plata["area_m2"] = buildings_in_la_plata.geometry.area

# Calculate total built area
total_built_area_m2 = buildings_in_la_plata["area_m2"].sum()
total_built_area_km2 = total_built_area_m2 / 1_000_000

print(f"Total building footprint area: {total_built_area_km2:.2f} km²")
print(f"Average building footprint: {buildings_in_la_plata['area_m2'].mean():.1f} m²")
```
...
Zonal statistics are not limited to continuous data, either. Below, we aggregate the Mapbiomas data to assess relative composition of la Plata's land cover by category.
